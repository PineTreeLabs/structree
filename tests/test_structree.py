"""Unit tests for structree.

Story-free, closed-form / by-construction assertions on the pytree contract:
flatten/unflatten roundtrips, the data-vs-static field split, ``replace``,
``__post_init__`` re-execution on unflatten, and the numpy ``ravel`` roundtrip.
"""
...
class State:
    pos: np.ndarray
    vel: np.ndarray
    mass: float = st.field(static=True, default=1.0)


@st.struct
class Reservoir:
    """Mirrors the autodiff.md EulerReservoirInletBC case: a struct that
    precomputes derived quantities in __post_init__. unflatten must rebuild the
    object so the derived field stays consistent with the leaves."""
...
def test_unflatten_roundtrip_preserves_static():
    s = State(np.array([0.0, 1.0]), np.array([2.0, 3.0]), mass=5.0)
    leaves, treedef = st.flatten(s)
    s2 = st.unflatten(treedef, [2 * x for x in leaves])
...
def test_nested_container_roundtrip():
    ...
    leaves, treedef = st.flatten(tree)
    assert len(leaves) == 2
    out = st.unflatten(treedef, leaves)
    assert out.keys() == tree.keys()
    assert np.allclose(out["b"]["c"], [3.0])


def test_jax_node_contract_children_aux_shape():
...
def test_is_struct():
    assert st.is_struct(State(np.zeros(1), np.zeros(1)))
    assert not st.is_struct({"a": 1})
    assert not st.is_struct(np.zeros(3))


def test_map_skips_static():
    s = State(np.array([1.0]), np.array([2.0]), mass=4.0)
    out = st.map(lambda x: x * 10, s)
...
def test_unflatten_reruns_post_init():
    ...
    assert r.rho0 == 5.0
    leaves, treedef = st.flatten(r)
    # change p0 leaf; rho0 must be recomputed on reconstruction
    r2 = st.unflatten(treedef, [20.0, 2.0])
    assert r2.p0 == 20.0
...
def test_ravel_dict_roundtrip():
    tree = {"pos": np.array([0.0, 1.0, 2.0]), "vel": np.array([3.0, 4.0, 5.0])}
    flat, unravel = st.ravel(tree)
